/*
  # Fix Timer Stop with Proper UUID Handling

  1. Problem
    - PostgreSQL doesn't have MIN() for UUID types
    - Need to get the oldest ticket ID for busy technicians

  2. Solution
    - Use DISTINCT ON to get the oldest ticket per employee
    - Order by opened_at to get the oldest first
    - This gives us the ticket ID that should be completed when assigning to new ticket

  3. Key Changes
    - employee_open_tickets CTE now uses DISTINCT ON pattern
    - Excludes tickets with completed_at IS NOT NULL
    - Returns oldest_ticket_id for busy technicians to use
*/
